    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for num in range(3):
                ...
                loss = 0.0
                if num == 0:
                    outputs1, outputs2, masks, stage_out1, _ = model(image_batch, [])
                else:
                    outputs1, outputs2, masks, stage_out1, _ = model(image_batch, en)
                en = []
                ...
                for s in ss:
                    ...
                    for idx in range(len(s)):
                        iout += stage_out1[s[idx]]
                    loss_agg += (lc1 * ce_loss(iout, label_batch[:].long()) + lc2 * dice_loss(iout, label_batch, softmax=True))
                    
                loss_outputs1 = lc1 * ce_loss(outputs1, label_batch[:].long()) + lc2 * dice_loss(outputs1, label_batch, softmax=True)
                loss_outputs2 = lc1 * ce_loss(outputs2, label_batch[:].long()) + lc2 * dice_loss(outputs2, label_batch, softmax=True)
                loss = loss_agg + loss_outputs1 + loss_outputs2
...
#                 loss += loss_outputs1 + loss_outputs2 + (loss1 + loss2 + loss3 + loss4 + loss5) * 0.7
        ...
        if epoch_num >= 0:
            feature_tensor = iout
            selected_feature_map = feature_tensor[0, 0, :, :].detach().cpu().numpy()

            plt.figure(figsize=(12, 9))
            plt.imshow(selected_feature_map, cmap='viridis', interpolation='lanczos')  # 使用bicubic插值提高图像清晰度
            plt.colorbar()  # 显示颜色条
